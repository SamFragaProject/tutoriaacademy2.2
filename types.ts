...
export interface AdminUser {
  ...
  activo: boolean;
  ultimo_acceso?: string; // ISO Date
  fecha_creacion: string; // ISO Date
  grupos?: string[]; // para profesores
  total_grupos?: number;
  total_alumnos?: number; // para profesores
}
...
export interface AdminStats {
  ...
  tasaActivacion: number; // porcentaje
  usuariosPorRol: {
    admin: number;
    director: number;
    docente: number;
  ...
  tendencias: {
    usuariosNuevos: Array<{ fecha: string; cantidad: number }>;
    actividadDiaria: Array<{ fecha: string; activos: number }>;
  };
}
...
export interface AdminActivity {
  ...
  admin_id: string;
  admin_nombre: string;
  accion: 'crear' | 'editar' | 'eliminar' | 'activar' | 'desactivar' | 'asignar';
  entidad: 'usuario' | 'escuela' | 'grupo';
  entidad_id: string;
